...
Для подключения билиотеки к своему файлу, подключите заголовочный файл `s21_string.h`, указав корректные пути, при компиляции подключайте статическую библиотеку. Пример:
```bash
